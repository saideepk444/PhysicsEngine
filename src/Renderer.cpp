...
void Renderer::render(const PhysicsWorld &world)
{
    ...
    for (const auto &body : world.getRigidBodies())
    {
        ...
        if (auto sphereCollider = std::dynamic_pointer_cast<SphereCollider>(body->getCollider()))
        {
            drawSphere(body->getPosition(), sphereCollider->getRadius());
        }
        // Add more collider types here
    }
}
